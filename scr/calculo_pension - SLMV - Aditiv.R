...
######################PARAMETROS################################################
#SMLV          <- 1310800
#SMLV          <- 1392000#1310800 # 1160000 #1158351 #1392000
SMLV          <- 1456910 
ahno_valuacion<- 2024
#tipo          <- "con_desl_cam_mesa" #"sin_desl_cam_mesa"#"sin_desl_fch_corte"
tipo          <- "sin_desl_cam_mesa_I_aditiv" #"con_desl_cam_mesa2" #  
#tipo<-"sin_desl_fch_corte"#"con_desl_fch_corte"##
######################INSUMOS###################################################
base_formato_394 <- read_excel("data/FT394_ENE_2024 - cambio 12_SMMLV.xlsx" , sheet = "fecha_corte",
                               #base_formato_394 <- read_excel("data/FT394_ENE_2023 - cambio 20_SMMLV.xlsx" , sheet = "fecha_corte",
                               ...
                               col_types = c("text", "text", "text",
                                             ...
                                             "text", "text", "text",
                                             "numeric", "numeric")
) #%>% filter(COMPANIA%in%c("ALFA","ALLIANZ") )

Tabla_validos_M <- read_excel("data/Tabla_validos_M.xlsx")
Tabla_validos_F <- read_excel("data/Tabla_validos_F.xlsx")
Tabla_SMLV <- read_excel("data/SMLV.xlsx")
Tabla_nueva <- read_excel("data/tabla_nueva_2020_final.xlsx")
######################CALCULO TABLA INVALIDOS###################################
source("scr/crear_tabla_invalidos.R")
################################################################################
invalidos_M <- read_excel("data/Tabla_invalidos_M.xlsx")
invalidos_F <- read_excel("data/Tabla_invalidos_F.xlsx")
#########################FIX MORTALITY##########################################
...
deslizamiento_esc0 = 0.00/100#/1.67/100#0/100 #(aumento SLM)
deslizamiento_esc1 = deslizamiento_esc0 - 0.001 #(disminucion SLM)
deslizamiento_esc2 = deslizamiento_esc0 + 0.001 #(aumento SLM)
tabla_incremento_slmv <- tibble(ahno = ahno_valuacion+1:3000) %>%
  mutate(crecimiento_anual_IPC =
           case_when(ahno ==ahno_valuacion+1 ~ 1,
                     ahno ==ahno_valuacion+2 ~ 1+3.66/100,#3.91/100,#8.07/100,
                     ahno ==ahno_valuacion+3 ~ 1+3.46/100,#3.90/100,#4.20/100,
                     TRUE ~ 1+3.00/100
           ),
         crecimiento_anual_SLM_ESC0 = case_when(
           ahno ==   ahno_valuacion+1 ~ 1,
           TRUE ~ crecimiento_anual_IPC +deslizamiento_esc0
         ),
         crecimiento_anual_SLM_ESC1 = case_when(
           ahno ==   ahno_valuacion+1 ~ 1,
           TRUE ~ crecimiento_anual_IPC +deslizamiento_esc1 
         ) ,
         crecimiento_anual_SLM_ESC2 = case_when(
           ahno ==   ahno_valuacion+1 ~ 1,
           TRUE ~ crecimiento_anual_IPC +deslizamiento_esc2 
         )
  ) %>% 
...
)
{
  ...
  retorno <- 
    ...
    mutate(interes_tec_mens = (1 + interes_tecnico)^(1/12),
           ipc_mens = (crecimiento_anual_IPC)^(1/12),   
           ipc_mens_lagged = lead(ipc_mens),
           inf_tec = 1/(interes_tec_mens*ipc_mens_lagged)
    ) %>% 
    mutate(v = cumprod(inf_tec)) %>% 
    ...
    select(t, mes, ahno, tasa_crecimiento, gastos, crecimiento_anual_IPC, crecimiento_acum_IPC,
           ...
           mesada_total_gatos_SLM_ESC2, mesada_total_gatos_SLM_ESC_IPC, mesada_1
    ) %>% 
...
################################################################################
pensiones_RL$TEMPORALIDAD <- as.numeric(pensiones_RL$TEMPORALIDAD)
pensiones_RL$FECHACORTE<-as.Date(pensiones_RL$FECHACORTE)
#pensiones_RL$FECHACORTE<-as.Date("2023-01-31")
  ...
  mutate(tabla_for_calculate = 
           map(tabla_final_D,
               ~.x %>% 
                 ...
                 mutate(monto_Aux_Fun = min(10*SMLV,max(mesada_1, 5*SMLV)),
  ...
  mutate(resumen_final_pension = map(tabla_for_calculate,
                                     ~.x %>% 
                                       ...
                                       summarise(reser_vejez_ESC0 = sum(t_p1*mesada_total_gatos_SLM_ESC0*v, na.rm = T),
                                                 ...
                                                 reser_auxfun_ESC0 = sum(Fact_AF_A*Monto_AF_Gastos*v, na.rm = T),
                                                 reser_auxfun_ESC1 = sum(Fact_AF_A*Monto_AF_Gastos*v, na.rm = T),  
                                                 reser_auxfun_ESC2 = sum(Fact_AF_A*Monto_AF_Gastos*v, na.rm = T),
                                                 reser_auxfun_ESC_IPC = sum(Fact_AF_A*Monto_AF_Gastos*v, na.rm = T)
                                       ) 
  )
  )
tictoc::toc()
...
openxlsx::write.xlsx(hojas,paste0("outputs/excel/comparacion_reserva_total_",ahno_valuacion,"_",tipo,".xlsx"), overwrite = TRUE)
#########
set.seed(123)
...
vr = cc %>% 
  ...
  summarise(reser_vejez_ESC0 = sum(t_p1*mesada_total_gatos_SLM_ESC0, na.rm = T),
            ...
            reser_sobre_ESC_IPC = sum(pi_sob*mesada_total_gatos_SLM_ESC_IPC*v, na.rm = T),
            reser_auxfun_ESC0 = sum(Fact_AF_A*Monto_AF_Gastos, na.rm = T),
            reser_auxfun_ESC1 = sum(Fact_AF_A*Monto_AF_Gastos, na.rm = T),  
            reser_auxfun_ESC2 = sum(Fact_AF_A*Monto_AF_Gastos, na.rm = T),
            reser_auxfun_ESC_IPC = sum(Fact_AF_A*Monto_AF_Gastos, na.rm = T)
  ) %>% 
  ungroup() %>% 
  mutate(ipc_TOTAL  = rowSums(select(.,contains("IPC"))),
         ESC0_TOTAL = rowSums(select(.,contains("ESC0"))),
...
#### GUARDAR FLUJOS ANUALES
openxlsx::write.xlsx(vr, paste0("outputs/flujo_anual_pensiones_vr",ahno_valuacion,"_",tipo,".xlsx"))
#colnames(dd)
