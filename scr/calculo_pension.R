...
tipo<- "sin_desliza_1_2024"#
#tipo<- "con_desl_IPC"
#"base"##"con_desliza"#"sin_desl_IPC"##"con_desl_IPC"
######################INSUMOS###################################################
base_formato_394 <- read_excel("data/FT394_ENE_2024 - VF.xlsx" , sheet = "Sheet 1",
...
deslizamiento_esc0 = 0/100#0/100 #(aumento SLM)
               ...
               ~.x %>% 
                 ...
                 mutate(
                        monto_Aux_Fun         = min(10*SMLV,max(mesada_1, 5*SMLV)),
                          monto_Aux_Fun_ESC0    = min(10*SMLV,max(mesada_1_cont_SLM_ESC0, 5*SMLV)),
                          monto_Aux_Fun_ESC1    = min(10*SMLV,max(mesada_1_cont_SLM_ESC1, 5*SMLV)),
                          monto_Aux_Fun_ESC2    = min(10*SMLV,max(mesada_1_cont_SLM_ESC2, 5*SMLV)),
                          monto_Aux_Fun_ESC_IPC = min(10*SMLV,max(mesada_1_cont_SLM_ESC_IPC, 5*SMLV)),
                        #
...
vr = cc %>% 
  ...
  summarise(reser_vejez_ESC0 = sum(t_p1*mesada_total_gatos_SLM_ESC0, na.rm = T),
            ...
            reser_auxfun_ESC_IPC = sum(Fact_AF_A*Monto_AF_Gastos_ESC0_IPC, na.rm = T)
  ) %>% 
  ungroup() %>% 
  mutate(ipc_TOTAL  = rowSums(select(.,contains("IPC"))),
         ESC0_TOTAL = rowSums(select(.,contains("ESC0"))),
