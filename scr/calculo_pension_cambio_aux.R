...
)#[1:10,] #%>% filter(COMPANIA%in%c("ALFA","ALLIANZ") )
#### ------- #####
...
deslizamiento_esc0 = 00/100#0/100 #(aumento SLM)
...
tabla_incremento_slmv <- tibble(ahno = ahno_valuacion:3000) %>%
  mutate(crecimiento_anual_IPC =
           case_when(ahno == ahno_valuacion    ~ 1 ,
                     ahno == ahno_valuacion +1 ~ 1 + 5.45/100,#3.91/100,#,#3.91/100,#7.39,#4.07/100,#4.63/100,#6.74/100, # corte sep 2022 # 7.39/100,#oct,#
                     ahno == ahno_valuacion +2 ~ 1 + 3.46/100,#3.90/100,#,#3.90/100,#4.01,# 3.15/100,#3.41/100,#3.95/100, # corte sep 2022 #4.01/100,#oct
                     TRUE ~  1+3/100
           ),
         ...
         crecimiento_anual_SLM_ESC2 = case_when(
           ahno == ahno_valuacion ~ 1 ,#11.15/100,#0.0,
...
)
{
  ...
  retorno <- 
    ...
    mutate(
      ...
      mesada_1_ESC0    = (mesada * ((1 + tasa_crecimiento)*crecimiento_acum_SLM_ESC0) ^ pot),
      mesada_1_ESC1    = (mesada * (1 + tasa_crecimiento) ^ pot)*crecimiento_acum_SLM_ESC1,
      mesada_1_ESC2    = (mesada * (1 + tasa_crecimiento) ^ pot)*crecimiento_acum_SLM_ESC2,
      mesada_1_ESC_IPC = (mesada * (1 + tasa_crecimiento) ^ pot)*crecimiento_acum_IPC,
      #
      mesada_adicional =
        case_when(
    ...
    select(t, mes, ahno, tasa_crecimiento, gastos, crecimiento_anual_IPC, crecimiento_acum_IPC,
           ...
           mesada_total_SLM_ESC0,mesada_total_SLM_ESC1,mesada_total_SLM_ESC2,mesada_total_SLM_ESC_IPC,
           mesada_1,mesada_1_ESC0,mesada_1_ESC1,mesada_1_ESC2,mesada_1_ESC_IPC,
           mesada_1_cont_SLM_ESC0,mesada_1_cont_SLM_ESC1,mesada_1_cont_SLM_ESC2,mesada_1_cont_SLM_ESC_IPC
  ...
  mutate(tabla_for_calculate = 
           map(tabla_final_D,
               ~.x %>% 
                 ...
                 mutate(
                   monto_Aux_Fun         =  case_when(estado == 2 ~min(10*SMLV,max(mesada_1, 5*SMLV)),TRUE ~0 ),
                   monto_Aux_Fun_ESC0    =  case_when(estado == 2 ~ min(10*SMLV,max(mesada_total_SLM_ESC0, 5*SMLV)),TRUE ~0 ),
                   monto_Aux_Fun_ESC1    =  case_when(estado == 2 ~  min(10*SMLV,max(mesada_total_SLM_ESC1, 5*SMLV)),TRUE ~0),
                   monto_Aux_Fun_ESC2    =  case_when(estado == 2 ~  min(10*SMLV,max(mesada_total_SLM_ESC2, 5*SMLV)),TRUE ~0),
                   monto_Aux_Fun_ESC_IPC =  case_when(estado == 2 ~  min(10*SMLV,max(mesada_total_SLM_ESC_IPC, 5*SMLV)),TRUE ~0),
                   #
                                     ...
                                     ~.x %>% 
                                       ...
                                       summarise(reser_vejez_ESC0 = sum(t_p1*mesada_total_gatos_SLM_ESC0*v, na.rm = T),
                                                 ...
                                                 reser_sobre_ESC_IPC = sum(pi_sob*mesada_total_gatos_SLM_ESC_IPC*v, na.rm = T),
                                                  reser_auxfun = sum(Fact_AF_A*Monto_AF_Gastos*v, na.rm = T), # todos los escenarios iguales
...
write.xlsx(hojas,paste0("outputs/excel/comparacion_reserva_total_",ahno_valuacion,"_",tipo,"correcion_aux.xlsx"), overwrite = TRUE)
#########
set.seed(123)
...
write_delim(dd, paste0("outputs/flujo_anual_pensiones",ahno_valuacion,"_",tipo,"_aux.txt"),delim = "|")

write.xlsx(dd, paste0("outputs/flujo_anual_pensiones_dd",ahno_valuacion,"_",tipo,"_correcion_aux.xlsx"))
#colnames(dd)
...
#### GUARDAR FLUJOS ANUALES
write.xlsx(vr, paste0("outputs/flujo_anual_pensiones_vr",ahno_valuacion,"_",tipo,"_correcion_aux.xlsx"))
#colnames(dd)
