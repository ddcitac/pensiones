...
deslizamiento_esc0 = 1.41/100#0/100 #(aumento SLM)
...
tabla_incremento_slmv <- tibble(ahno = ahno_valuacion:3000) %>%
  mutate(crecimiento_anual_IPC =
           case_when(ahno == ahno_valuacion    ~ 1 ,
                     ...
                     ahno == ahno_valuacion +2 ~ 1 + 3.66/100,#3.90/100,#,#3.90/100,#4.01,# 3.15/100,#3.41/100,#3.95/100, # corte sep 2022 #4.01/100,#oct
                     TRUE ~  1+3/100
           ),
         crecimiento_anual_SLM_ESC0 = case_when(
           ahno == ahno_valuacion ~ 1 ,
           TRUE ~ crecimiento_anual_IPC +(0+deslizamiento_esc0)
         ),
         crecimiento_anual_SLM_ESC1 = case_when(
           ahno == ahno_valuacion ~ 1 ,
           TRUE ~ crecimiento_anual_IPC +(0+deslizamiento_esc1) 
         ) ,
         crecimiento_anual_SLM_ESC2 = case_when(
           ahno == ahno_valuacion ~ 1 ,#11.15/100,#0.0,
           TRUE ~ crecimiento_anual_IPC + (0+deslizamiento_esc2) 
         )
  ) %>% 
...
write.xlsx(hojas,paste0("outputs/excel/comparacion_reserva_total_",ahno_valuacion,"_",tipo,"_correccion.xlsx"), overwrite = TRUE)
#########
set.seed(123)
...
write.xlsx(dd, paste0("outputs/flujo_anual_pensiones_dd",ahno_valuacion,"_",tipo,"_correccion.xlsx"))
#colnames(dd)
...
#### GUARDAR FLUJOS ANUALES
write.xlsx(vr, paste0("outputs/flujo_anual_pensiones_vr",ahno_valuacion,"_",tipo,"_correccion.xlsx"))
#colnames(dd)
